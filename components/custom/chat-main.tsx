...
export function Chat({
  ...
  const { attachmentFile, setAttachmentFile, handleFileAdded, handleFileRemoved } =
    useFileHandler(textareaRef)

  // custom hook for handling messages and API calls
  const { messages, isResponding, input, setInput, sendMessage } = useChatMessages({
    user_id: user.id,
  ...
  const { isDraggingOver, handlers } = useFileInput(
    textareaRef,
    setInput,
    handleFileAdded,
    attachmentFile
  ...
  return (
    <div
      className="absolute inset-0 flex flex-col overflow-hidden transition-colors duration-200 ease-in-out md:border-t"
      {...handlers}
    >
      ...
      <div
        className="flex-1 overflow-y-auto scrollbar-hide min-h-0 pt-4"
