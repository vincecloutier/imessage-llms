    ...
    switch (field.type) {
      case 'text':
        fieldSchema = z
          ...
          .refine((val) => val.trim() !== '', 'Input cannot be whitespace only')
        break
      case 'long_text':
        fieldSchema = z
          .string()
      ...
      case 'calendar':
        ...
        fieldSchema = z
          .date({ required_error: 'This field is required', invalid_type_error: 'Invalid date' })
          .max(eighteenYearsAgo, { message: 'Must be at least 18 years old' })
        break
      case 'enum':
        ...
        fieldSchema = z.preprocess(
          ...
          z
            ...
            .refine(
              ...
              {
                message:
                  'Invalid email or phone number format. Phone numbers must start with + and contain only digits.',
              }
            )
            .nullable()
            .optional()
      ...
      default:
        fieldSchema = z
          .any()
          .refine((val) => val !== undefined && val !== null, { message: 'This field is required' })
    }
    shape[field.name] = fieldSchema
  })
  shape['telegram_username'] = z
    .string()
    .refine((val) => val === null || val === undefined || !val.includes('@'), {
  ...
  return baseSchema.transform((data) => {
    ...
    fields.forEach((field) => {
      ...
      if (field.type === 'calendar' && value instanceof Date) {
        transformedData[fieldName] = value.toISOString().slice(0, 10)
      }
...
export default function GenericForm({
    ...
    allFields.forEach((field) => {
      ...
      switch (field.type) {
        ...
        case 'calendar':
          ...
          break
        case 'location':
          const existingLocation = attrs['location']
            ? {
                name: attrs['location'],
                lat: attrs['latitude'],
                lon: attrs['longitude'],
          ...
          value =
            existingLocation ??
            (isNewRecord ? undefined : { name: '', lat: 0, lon: 0, timezone: '' })
          break
        case 'messaging_platform':
          value = {
  ...
  const onSubmit = async (data: any) => {
    ...
    await toast.promise(
        ...
        }),
      {
        ...
        error: (err) => `Failed to save changes: ${err.message || 'unknown error.'}`,
      }
    )
    router.push('/chat/0')
    router.refresh()
  ...
  const renderFormField = (field: FieldSchema, ctrl: any) => {
    ...
    switch (field.type) {
      ...
      case 'long_text':
        return <Textarea {...ctrl} value={safeValue} rows={10} className="resize-none" />
      case 'number':
        return (
          <Input
            type="number"
            ...
            value={safeValue}
            onChange={(e) =>
              ctrl.onChange(e.target.value === '' ? undefined : Number(e.target.value))
            }
          />
        )
      case 'calendar':
  ...
  const dialogWidthClass = 'sm:max-w-[60%] md:max-w-[50%] lg:max-w-[40%]'
  return (
    <Credenza open={open} onOpenChange={handleOpenChange} preventClose={forceAnswer}>
      ...
      >
                    ...
                    <FormField
                      ...
                      render={({ field: ctrl }) => (
                        <FormItem style={{ gridColumn: 'span 1' }}>
                          ...
                          <FormControl>{renderFormField(field, ctrl)}</FormControl>
                          {field.description && (
        ...
        <CredenzaFooter>
          <div
            className={`flex w-full gap-4 ${destructiveButton ? 'justify-between' : 'justify-end'}`}
          >
            {destructiveButton}
            <Button
              type="submit"
              form="generic-dialog-form"
