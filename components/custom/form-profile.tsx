...
const profileFields: FieldSchema[] = [
  ...
  {
    name: 'sender_address',
    label: 'iMessage Address (Optional)',
    description: 'The phone number or email address you use with iMessages.',
    rowId: 'a4',
    type: 'sender_address',
  },
  {
    ...
    description: 'The username you use with Telegram (without the @).',
    rowId: 'a5',
    type: 'text',
  },
]
...
export function ProfileForm({ user, profile }: { user: User; profile: Profile | null }) {
  ...
  const defaultValues = useMemo(
    () => profile || { id: user?.id, attributes: {}, sender_address: '' },
    [profile, user]
  )
        ...
        >
          <Avatar className="size-8 rounded-lg bg-indigo-600 text-white overflow-hidden">
            <AvatarFallback className="rounded-lg bg-transparent">
              {profile.attributes?.name as string}
            </AvatarFallback>
          </Avatar>
