...
def responder():
    ...
    # save user message
    save_message(
        profile["id"], persona["id"], channel, "user", message, attachment_data
    )
    ...
    response = llm_call(
        ...
        profile["id"],
        persona["id"],
        get_messages(profile["id"], persona["id"], channel),
    )
    ...
    # for the web channel
    return jsonify(
        {"status": 200, "message": {"role": "assistant", "content": response}}
    )
