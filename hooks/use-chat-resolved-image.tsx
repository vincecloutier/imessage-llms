...
export function useResolvedImageUrl(source: string | File | null | undefined): {resolvedUrl: string | null, isLoading: boolean, error: string | null} {
  ...
  useEffect(() => {
    ...
    const resolveSource = async () => {
      ...
      if (source instanceof File) {
        ...
        if (isMounted) {setResolvedUrl(objectUrlToRevoke);}
      }
      // handle string sources that start with blob: or http: by using them directly
      else if (typeof source === 'string' && (source.startsWith('blob:') || source.startsWith('http'))) {
        if (isMounted) {setResolvedUrl(source);}
      }
